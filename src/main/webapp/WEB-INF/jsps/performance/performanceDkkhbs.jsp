...
<html>
  ...
  <body >
  <div>
	  <form action="${pageContext.request.contextPath}/findPerformanceDkkhbsMx.action?yggh=${yggh}&type=${type}" method="post" 
		  ...
		  <button type="submit" class="btn btn-primary">查询</button>
		   ...
		   <c:forEach items="${list}" var="item">
		   	<tr>
		   		...
		   		<td>${item.ffrq}</td>
		   		<td>${item.dqrq}</td>
		   		<td align="right">${item.dkje}</td>
				<c:choose>
		   			<c:when test="${type eq 'bs'}">
		   				<td align="right">${item.bsdj}</td>
		   				<td align="right">${item.bsgz}</td>
		   			</c:when>
		   			<c:when test="${type eq 'ed'}">
		   				<td align="right">${item.eddj}</td>
		   				<td align="right">${item.edgz}</td>
		   			</c:when>
		   		</c:choose>
		   </tr>
		  </c:forEach>
  ...
  <script type="text/javascript">
	var url = '${pageContext.request.contextPath}/findPerformanceDkkhbsMx.action?yggh=${yggh}&type=${type}';
  	function jumpPage(){
