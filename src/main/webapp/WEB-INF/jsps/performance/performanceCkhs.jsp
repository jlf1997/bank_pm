...
<html>
  ...
  <body >
  <div>
	  <form action="${pageContext.request.contextPath}/findPerformanceCkhsMx.action?yggh=${yggh}&yxlx=${yxlx}" method="post" 
	  class="form-inline" role="form" style="margin-top:1%;margin-left:1%;" id="form">
		  ...
		  <div class="form-group">
		  	<input type="text" class="form-control" placeholder="客户名称" name="condition" value="${condition}"/>
		  </div>
		  <button type="submit" class="btn btn-primary">查询</button>
		   ...
		   <c:forEach items="${list}" var="item">
		   	<tr>
		   		...
		   		<td>
					<c:choose>
		   			<c:when test="${item.lx eq 1}">
		   				存量
		   			</c:when>
		   			<c:when test="${item.lx eq 2}">
		   				增量
		   			</c:when>
		   		</c:choose>
				</td>
...
<script type="text/javascript">
	var url = '${pageContext.request.contextPath}/findPerformanceCkhsMx.action?yggh=${yggh}&yxlx=${yxlx}';
  	function jumpPage(){
