...
<html>
  ...
  <body >
  <div>
	<form action="${pageContext.request.contextPath}/findPerformanceCkYzkhgzMx.action?yggh=${yggh}" method="post" 
	  class="form-inline" role="form" id="form" style="margin-top:1%;margin-left:1%;">
		  ...
		  <button type="submit" class="btn btn-primary">查询</button>
		   ...
		   <c:forEach items="${list}" var="item">
		   	<tr>
		   		...
		   		<td>${item.qmnrp}</td>
		   		<td>${item.yzkhdj}</td>
		   		<td align="right">${item.yzkhgz}</td>
		   		<td align="right">${item.yzkhljgz}</td>
		   	</tr>
		   	</c:forEach>
	</table>
	<!-- 分页 -->
...
<script type="text/javascript">
	var url = '${pageContext.request.contextPath}/findPerformanceCkYzkhgzMx.action?yggh=${yggh}';
  	function jumpPage(){
