...
<html>
  ...
  <body >
  <div>
	<form action="${pageContext.request.contextPath}/findZhhPerformanceCkAljcgzMx.action?type=${type}&zzbz=${zzbz}" method="post" 
  	...
  	<table class="table table-bordered table-striped table-hover table-condensed" style="width:98%;margin:0 auto;">
		...
		<c:forEach items="${list}" var="item">
			<tr>
		   		...
		   		<c:if test="${type eq 'cl'}">
		   			<td>${item.clyrpdj}</td>
		   			<td align="right">${item.clyrpgz}</td>
		   			<td align="right">${item.clyrpljgz}</td>
		   		</c:if>
		   		<c:if test="${type eq 'zl'}">
		   			<td>${item.zlyrp}</td>
		   			<td>${item.zlyrpdj}</td>
		   			<td align="right">${item.zlyrpgz}</td>
		   			<td align="right">${item.zlyrpljgz}</td>
		   		</c:if>
		   	</tr>
		   	</c:forEach>
	</table>
...
<script type="text/javascript">
	var url = '${pageContext.request.contextPath}/findZhhPerformanceCkAljcgzMx.action?type=${type}&zzbz=${zzbz}';
  	function jumpPage(){
