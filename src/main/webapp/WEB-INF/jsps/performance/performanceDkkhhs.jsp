...
<html>
  ...
  <body >
  <div>
	<form action="${pageContext.request.contextPath}/findPerformanceDkKhhsMx.action?yggh=${yggh}&lx=${lx}" method="post" 
	  class="form-inline" role="form" style="margin-top:1%;margin-left:1%;" id="form">
		  ...
		  <div class="form-group">
		  	<input type="text" class="form-control" placeholder="客户名称|证件号码" name="condition" value="${condition}"/>
		  </div>
  		...
  		<table class="table table-bordered table-striped table-hover table-condensed" style="width:98%;margin:0 auto;">
		   ...
		   <c:forEach items="${list}" var="item">
		   	<tr>
		   		...
		   		<td>${item.zzkhr}</td>
		   		<td>
		   		<c:if test="${item.lx eq 1 }">
		   			存量
		   		</c:if>
		   		<c:if test="${item.lx eq 2}">
		   			增量
		   		</c:if>
		   		</td>
		   		<td>${item.dj}</td>
	...
	<div style="width:100%;">
	<ul class="pagination" style="margin:1% 1%;;">
		...
		<!-- 进行每一个页面显示数据条目大小的选择 -->
		<li>
			<select title='显示条数' style="width:7%;height:5%;float:left;margin-top:3px;" onchange='onShowCountChange(this.value);'>
...
<script type="text/javascript">
	var url = '${pageContext.request.contextPath}/findPerformanceDkKhhsMx.action?yggh=${yggh}&lx=${lx}';
  	function jumpPage(){
