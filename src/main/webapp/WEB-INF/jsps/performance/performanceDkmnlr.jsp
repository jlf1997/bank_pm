...
<html>
  ...
  <body >
  <div>
	  <form action="${pageContext.request.contextPath}/findPerformanceDkmnlrgzMx.action?yggh=${yggh}&type=${type}" method="post" 
	  class="form-inline" role="form" style="margin-top:1%;margin-left:1%;" id="form">
		  ...
		  <div class="form-group">
		  	<input type="text" class="form-control" placeholder="账号名称|贷款账号" name="condition" value="${condition}"/>
		  </div>
		  <button type="submit" class="btn btn-primary">查询</button>
		   ...
		   <c:forEach items="${list}" var="item">
		   	<tr>
		   		...
		   		<td>
			   		<c:choose>
			   			<c:when test="${item.wjflbz eq 1}">
			   				正常
			   			</c:when>
			   			<c:when test="${item.wjflbz eq 2}">
			   				关注
			   			</c:when>
			   			<c:when test="${item.wjflbz eq 3}">
			   				次级
			   			</c:when>
			   			<c:when test="${item.wjflbz eq 4}">
			   				可疑
			   			</c:when>
			   			<c:when test="${item.wjflbz eq 5}">
			   				损失
			   			</c:when>
			   			<c:otherwise>
			   				未知
		   		...
		   		<td>
			   		<c:choose>
			   			<c:when test="${item.ncwjflbz eq 1}">
			   				正常
			   			</c:when>
			   			<c:when test="${item.ncwjflbz eq 2}">
			   				关注
			   			</c:when>
			   			<c:when test="${item.ncwjflbz eq 3}">
			   				次级
			   			</c:when>
			   			<c:when test="${item.ncwjflbz eq 4}">
			   				可疑
			   			</c:when>
			   			<c:when test="${item.ncwjflbz eq 5}">
			   				损失
			   			</c:when>
			   			<c:otherwise>
			   				未知
		   		...
		   		<td align="right">${item.clmnlr}</td>
		   		<td align="right">${item.qmmnlr}</td>
		   		<c:choose>
		   			<c:when test="${type eq 'cl'}">
		   				<td align="right">${item.dkye}</td>
...
<script type="text/javascript">
	var url = '${pageContext.request.contextPath}/findPerformanceDkmnlrgzMx.action?yggh=${yggh}&type=${type}';
  	function jumpPage(){
