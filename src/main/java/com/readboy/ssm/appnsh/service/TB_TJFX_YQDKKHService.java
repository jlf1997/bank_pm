...
public class TB_TJFX_YQDKKHService {
	...
	public int getCount(String yggh,int khlx) {
		String sql = "select * from tb_tjfx_yqdkkh where yggh=? and khlx=? order by yqts desc";
	...
	public Map getPages(String yggh, Integer khlx, Integer pageSize, Integer pageIndex) {
		...
		String sqlPage = "select tb.*,org.ZZJC as jgmc from tb_tjfx_yqdkkh tb "
				+ "left join hr_bas_organization org on tb.jgdm = org.YWJGDM  where yggh=? and khlx=? order by yqts desctb.khmc desc,tb.jgdm desc ";
