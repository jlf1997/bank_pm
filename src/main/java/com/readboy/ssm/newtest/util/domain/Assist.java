package com.readboy.ssm.newtest.util.domain;
...
/**
 * 类描述:配合MyBatis-CMEU_2.1.3的辅助类增强版
 * V1.0
 * 作者:wangyaqiang
 * 创建时间:2018年4月26日 下午3:19:38
 */
public class Assist {
	// 去重
	 ...
	 */
	public Assist setRequires(WhereRequire<?> whereRequire) {
	 ...
	 * @return
	 */
	public static <T> WhereRequire<T> andIn(String column, String[] values) {
		if (null == values || values.length == 0) {
			return new Assist().new WhereRequire<T>("and " + column + " in ", "");
		}
		...
		String req = sb.substring(0, sb.length() - 2) + ")";
	 ...
	 * @return
	 */
	public static <T> WhereRequire<T> andIn(String column, List<String> values) {
	 ...
	 * @return
	 */
	public static <T> WhereRequire<T> orIn(String column, String[] values) {
		if (null == values || values.length == 0) {
			return new Assist().new WhereRequire<T>("or " + column + " in ", "");
		}
		StringBuffer sb = new StringBuffer();
		sb.append("(");
		for (String value : values) {
		...
		String req = sb.substring(0, sb.length() - 2) + ")";
		return new Assist().new WhereRequire<T>("or " + column + " in ", req);
	}
	 ...
	 * @return
	 */
	public static <T> WhereRequire<T> orIn(String column, List<String> values) {
		if (null == values || values.size() == 0) {
			return new Assist().new WhereRequire<T>("or " + column + " in ", "");
		}
		StringBuffer sb = new StringBuffer();
	 ...
	 * @return
	 */
	public static <T> WhereRequire<T> andNull(String column, boolean isNull) {
		if (isNull) {
			return new Assist().new WhereRequire<T>("and " + column + " is ", "NULL");
		} else {
			return new Assist().new WhereRequire<T>("and " + column + " is not ", "NULL");
		}
	}
	 ...
	 * @return
	 */
	public static <T> WhereRequire<T> orNull(String column, boolean isNull) {
		if (isNull) {
			return new Assist().new WhereRequire<T>("or " + column + " is ", "NULL");
		} else {
			return new Assist().new WhereRequire<T>("or " + column + " is not ", "NULL");
		}
	}
	 ...
	 * 修改次数: 0
	 */
	private static <T> boolean empty(T value) {
		if (null == value) {
			return true;
		} else if (value.getClass().toString().indexOf("String") >= 0) {
